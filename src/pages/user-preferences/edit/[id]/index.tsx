...
import { getUserPreferenceById, updateUserPreferenceById } from 'apiSdk/user-preferences';
...
function UserPreferenceEditPage() {
  const router = useRouter();
  const id = router.query.id as string;
  const { data, error, isLoading, mutate } = useSWR<UserPreferenceInterface>(
    () => (id ? `/user-preferences/${id}` : null),
    () => getUserPreferenceById(id),
  );
  ...
  const handleSubmit = async (values: UserPreferenceInterface, { resetForm }: FormikHelpers<any>) => {
    setFormError(null);
    try {
      const updated = await updateUserPreferenceById(id, values);
      mutate(updated);
      resetForm();
