...
import { getUrls, deleteUrlById } from 'apiSdk/urls';
import { UrlInterface } from 'interfaces/url';

type ColumnType = ColumnDef<UrlInterface, unknown>;
...
export function UrlListPage(props: UrlListPageProps) {
  ...
  const fetcher = useCallback(
    async () =>
      getUrls({
        relations: ['organization', 'news.count'],
  ...
  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<UrlInterface>>(
    () => `/urls?params=${JSON.stringify(params)}`,
    fetcher,
  );
  ...
  const handleView = (row: UrlInterface) => {
    if (hasAccess('url', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/urls/view/${row.id}`);
    }
  };
  ...
  const columns: ColumnType[] = [
    ...
    hasAccess('organization', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ...
      : null,
    hasAccess('news', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ? {
          id: 'news',
          header: 'News',
          accessorKey: 'news',
          cell: ({ row: { original: record } }: any) => record?._count?.news,
        }
      : null,

    {
      ...
      cell: ({ row: { original: record } }: any) => (
        <Flex justifyContent="flex-end">
          <NextLink href={`/urls/view/${record.id}`} passHref legacyBehavior>
            <Button
              onClick={(e) => e.stopPropagation()}
              mr={2}
              padding="0rem 8px"
          ...
          {hasAccess('url', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
            <NextLink href={`/urls/edit/${record.id}`} passHref legacyBehavior>
              <Button
                onClick={(e) => e.stopPropagation()}
                mr={2}
                padding="0rem 0.5rem"
          ...
          {hasAccess('url', AccessOperationEnum.DELETE, AccessServiceEnum.PROJECT) && (
            <IconButton
              onClick={(e) => {
                e.stopPropagation();
    ...
    <Box p={4} rounded="md" shadow="none">
      <Flex justifyContent="space-between" mb={4}>
        <Flex alignItems="center" gap={1}>
          ...
          <AccessInfo entity="url" />
        </Flex>
        {hasAccess('url', AccessOperationEnum.CREATE, AccessServiceEnum.PROJECT) && (
          <NextLink href={`/urls/create`} passHref legacyBehavior>
            <Button
              onClick={(e) => e.stopPropagation()}
              height={'2rem'}
